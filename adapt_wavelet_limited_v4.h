...
This version is compatible with Basilisk version as of Nov 2nd 2023. 
Earlier versions became incompatible due to: https://groups.google.com/g/basilisk-fr/c/_57XuO20DS8/m/VYUsZHHwAQAJ

## Note that now, the calling of function is also different: 
scalar * slist = {f, u.x, u.y, tau_p.x.x, tau_p.x.y, tau_p.y.y, tau_qq, KAPPA, D2c, D33c};
double max[] = {fErr, VelErr, VelErr, StressErr, StressErr, StressErr, StressErr, KAPPAErr, EpsErr, EpsErr};
struct Adapt_limited args = { .slist = slist, .max = max, .MLFun = refRegion, .minlevel = 6};
adapt_wavelet_limited (&args);
